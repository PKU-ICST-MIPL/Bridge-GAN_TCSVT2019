...
tf.app.flags.DEFINE_string('checkpoint_dir',
                           './models/birds_valid299/model.ckpt',
...
def main(unused_argv=None):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            img_array = np.load(load_file)
            os.remove(load_file)
            
            mean, std = get_inception_score(sess, img_array, pred_op)
            
            with open(sys.argv[1].split('.pkl')[0] + '_record.txt', 'w') as f:
