"""Network architectures used in the GAN paper."""
...
#----------------------------------------------------------------------------
# Generator used in the Bridge-GAN paper.
...
def G_bridge(
    ...
    **kwargs):                                      # Arguments for sub-networks (G_mapping and G_synthesis).
    ...
    dlatents, _ = components.mapping.get_output_for(latents_in, labels_in, **kwargs)
    ...
    if style_mixing_prob is not None:
        with tf.name_scope('StyleMix'):
            ...
            dlatents2, _ = components.mapping.get_output_for(latents2, labels_in, **kwargs)
...
def G_mapping(
    ...
    **_kwargs):                             # Ignore unrecognized keyword args.
    ...
    dlatents = tf.identity(x, name='dlatents_out')
    dlatents_flatten = tf.layers.flatten(dlatents)
    with tf.variable_scope('G_mutual_info_layers'):
        mi_fc1 = layers.fully_connected(dlatents_flatten, num_outputs=128, activation_fn=leaky_rectify, normalizer_fn=layers.batch_norm, normalizer_params={"is_training":True})
        
        fix_std = True
        if fix_std:
            mi_fc2 = layers.fully_connected(mi_fc1, num_outputs=label_size, activation_fn=tf.identity)
        else:
            mi_fc2 = layers.fully_connected(mi_fc1, num_outputs=label_size*2, activation_fn=tf.identity)
    
    return dlatents, mi_fc2
...
def D_basic(
    ...
    structure           = 'auto',       # 'fixed' = no progressive growing, 'linear' = human-readable, 'recursive' = efficient, 'auto' = select automatically.
    is_template_graph   = False,        # True = template graph constructed by the Network class, False = actual evaluation.
    fix_std             = True,         # Fix continuous var standard deviation to 1.
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    with tf.variable_scope('D_mutual_info_layers'):
        mi_fc1 = layers.fully_connected(scores_out, num_outputs=128, activation_fn=leaky_rectify, normalizer_fn=layers.batch_norm, normalizer_params={"is_training":True})
