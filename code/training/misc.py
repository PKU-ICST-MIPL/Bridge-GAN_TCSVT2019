...
def setup_snapshot_image_grid(G, training_set,
    ...
    wrongs = np.zeros([gw * gh] + training_set.shape, dtype=training_set.dtype)
    labels = np.zeros([gw * gh, training_set.label_size], dtype=training_set.label_dtype)
    latents = np.random.randn(gw * gh, *G.input_shape[1:])

    # Random layout.
    if layout == 'random':
        reals[:], wrongs[:], labels[:] = training_set.get_minibatch_np(gw * gh)
    ...
    if layout in class_layouts:
        ...
        for _iter in range(1000000):
            real, wrong, label = training_set.get_minibatch_np(1)
        ...
        for i, block in enumerate(blocks):
            for j, (real, label) in enumerate(block):
                ...
                if x < gw and y < gh:
                    reals[x + y * gw] = real[0]
                    wrongs[x + y * gw] = wrong[0]
                    labels[x + y * gw] = label[0]

    return (gw, gh), reals, wrongs, labels, latents
